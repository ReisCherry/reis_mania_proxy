...
──────────────────────────────────────────────────────────────
  ...
  --bait <type>         Fishing bait type: auto | fish | meat | boot | none
                        (default: auto)

──────────────────────────────────────────────────────────────
  PERFORMANCE
──────────────────────────────────────────────────────────────
  --speed <ms>          Attack interval in ms (default: 400)
                        Lower = faster attacks (min ~200)
  --burst <n>           Attack packets per swing (default: 5, max: 10)
                        Each packet = 1 damage hit on server.
                        --burst 5  = 5x damage per swing
                        --burst 10 = near-instant kill on most mobs
...
──────────────────────────────────────────────────────────────
  ...
  # Boss hunting — max damage burst (1–5 hit kill)
  node bot.js --focus boss --skill single --stat str --speed 200 --burst 5 --fast

  # Monster farming — 10x damage per swing
  node bot.js --focus monster --speed 200 --burst 10 --fast
  ...
  try {
    ...
    for (let line of lines) {
      ...
      if (line && !line.startsWith('#')) {
        const agent = new ProxyAgent(line);
        // We will log this in main() to not clutter standard functions
        return { agent, url: line };
      }
    }
  } catch (err) {
    console.error(`❌ Error reading/parsing proxy.txt: ${err.message}`);
  }
  return null;
}

const proxyConfig = loadProxyAgent();
const proxyAgent = proxyConfig ? proxyConfig.agent : null;
...
const baitArg = arg('--bait', cfg.bait || 'auto').toLowerCase();
...
// ── Burst Damage ──
// --burst N: how many {t:'attack'} packets to send per swing.
// Server processes each packet as a separate damage hit with no dedup.
// burst=1 → normal damage. burst=5 → 5× damage. burst=10 → near-instant kill.
const ATK_BURST    = Math.min(10, Math.max(1, parseInt(arg('--burst', 5))));
...
function renderDashboard(state) {
  ...
  const goldAvail  = w.golds.filter(g => g.pct < 1000);
  const dmdAvail   = w.diamonds?.filter(d => d.pct < 1000) || [];
...
// ─────────────────────────────────────────────────────────────────────────────
// BURST DAMAGE sendAttack
// Exploit: server processes each {t:'attack'} packet as a separate damage hit
// with no dedup or per-burst rate limit. We send ATK_BURST packets per call.
//
// burst=1  → 1× (normal)
// burst=5  → 5× damage  (kills regular mobs in 1-2 swings)
// burst=10 → 10× damage (kills most bosses in 2-3 swings)
//
// Additionally: state packet always sends moving:false regardless of real state.
// This ensures the server's hit-registration check passes (stationary = hit lands).
// ─────────────────────────────────────────────────────────────────────────────
function sendAttack(tx, ty, burst) {
  const hitCount = burst !== undefined ? Math.min(10, Math.max(1, burst)) : ATK_BURST;
  ...
  // ── Force moving:false so server registers hit (stationary attack stance) ──
  // Real movement state is irrelevant — server only checks moving flag on the
  // state packet that immediately precedes the attack packets.
  const payload = {
    t: 'state',
    x: Math.round(state.player.x),
    y: Math.round(state.player.y),
    moving: false,              // ← Always false: ensures ALL burst hits land
    facing: state.player.facing > 0 ? 1 : -1,
    boat: state.player.boat,
    bd: bd,
    vcx: Math.round(state.player.x),
    vcy: Math.round(state.player.y),
    vr: 830,
  };
  send(payload);

  // ── Send N attack packets back-to-back (no delay between) ──
  // Each packet is a full damage hit. Server has no intra-burst dedup.
  for (let i = 0; i < hitCount; i++) {
    send({ t: 'attack' });
    state.session.attacks++;
    state.attacksOnCurrentTarget++;
  }
  log('⚔️', `Burst x${hitCount} | moving:false | facing:${bd}`);
}
...
function checkEquip() {
  ...
  const isFishingMode = state.mode === 'fishing';
  ...
  for (const item of items) {
    ...
    const isRod = itemName.includes('rod') || itemId.includes('rod') || itemName.includes('fish') || itemId.includes('fish');

    if (isFishingMode) {
      // In fishing mode, we want to equip a fishing rod (either in weapon or tool slot)
      if (isRod) {
    ...
    // Skip fishing rods when NOT in fishing mode
    if (isRod) continue;
  ...
  for (const slot in bestBySlot) {
    ...
    if (isFishingMode || slot === wantSlot || bestScore > currentScore) {
...
function doFishing() {
  ...
  // Deterministically select a spot based on BOT_NAME or WALLET to prevent bots from grouping
  if (state.selectedSpotIndex === null) {
    const identifier = BOT_NAME || WALLET || 'default';
    let hash = 0;
    for (let i = 0; i < identifier.length; i++) {
      hash = identifier.charCodeAt(i) + ((hash << 5) - hash);
    }
    state.selectedSpotIndex = Math.abs(hash) % spots.length;
    state.lastSpotChangeTime = Date.now();
  }

  // Cycle spot index if 60 minutes (3,600,000 ms) have passed
  const durationMs = 60 * 60 * 1000;
  if (state.lastSpotChangeTime && Date.now() - state.lastSpotChangeTime > durationMs) {
    const spotsCount = spots.length;
    let nextIndex = Math.floor(Math.random() * spotsCount);
    if (spotsCount > 1) {
      while (nextIndex === state.selectedSpotIndex) {
        nextIndex = Math.floor(Math.random() * spotsCount);
      }
    }
    state.selectedSpotIndex = nextIndex;
    state.lastSpotChangeTime = Date.now();
    log('🔄', `60 minutes elapsed! Cycling to another spot index: #${state.selectedSpotIndex + 1}`);
  }

  const spotIndex = state.selectedSpotIndex;
  const spot = spots[spotIndex];

  const targetX = toWorldCenter(spot.x);
  const targetY = toWorldCenter(spot.y);
  const d = distTo(targetX, targetY);

  if (d > 5) {
    if (Date.now() - (state._lastTargetLog || 0) > 5000) {
       log('🎯', `WALKING TO: Fishing Spot #${spotIndex + 1} [${spot.x}, ${spot.y}] (${targetX}, ${targetY}) | dist=${Math.round(d)}px`);
       state._lastTargetLog = Date.now();
    }
    ...
    if (landCache.size >= 30) {
      state.player.boat = !isLikelyLand(state.player.x, state.player.y);
    }
    ...
    if (!state._fishLogTime || Date.now() - state._fishLogTime > 5000) {
      log('🎣', `FISHING at Spot #${spotIndex + 1} [${spot.x}, ${spot.y}]...`);
    ...
    let bait;
    if (baitArg === 'auto') {
      bait = (state.inv.fish || 0) > 0 ? 'fish' : 'meat';
    } else if (baitArg === 'none') {
      bait = '';
    } else {
      bait = baitArg === 'boot' ? 'old_shoe' : baitArg;
    }
...
function doMonster(bossOnly = false) {
  ...
  if (!bossOnly && pool.length === 0) {
    ...
    if (farNormalMobs.length > 0) {
      const near = farNormalMobs.reduce((b, m) => distTo(m.x, m.y) < distTo(b.x, b.y) ? m : b, farNormalMobs[0]);
      log('⚔️', `Far mob detected: ${near.type} at dist ${distTo(near.x, near.y).toFixed(0)}`);
      moveToward(near.x, near.y, 80, 20);
      return;
    }
  }
  ...
  if (state.lockMobId) {
    ...
    if (cur) {
      ...
      if (d > targetDist) {
        moveToward(cur.x, cur.y, 80, targetDist - 10);
  ...
  if (pool.length > 0) {
    ...
    if (d > targetDist) {
      moveToward(near.x, near.y, 80, targetDist - 10);
      if (d <= 65) {
        faceTarget(near.x);
    ...
    // Wander a large distance (600px) to force the server to load new chunks and discover more mobs!
...
function attackTick() {
  ...
  switch (state.mode) {
    ...
    case 'fishing': {
      const nearbyMob = state.world.mobs.find(m => {
        if (m.state === 'dead' || m.hp <= 0) return false;
        const mType = (m.type || '').toLowerCase();
        if (mType && skipMobs.includes(mType)) return false;
        return distTo(m.x, m.y) < 200;
      });

      if (nearbyMob) {
        log('🛡️', `Self-Defense: Monster detected nearby (${nearbyMob.type})! Fighting...`);
        // If we are currently fishing, stop fishing first
        if (state._lastFishTime && Date.now() - state._lastFishTime < 10000) {
          send({ t: 'fishStop' });
        }
        state.lockMobId = nearbyMob.id;
        doMonster(nearbyMob.boss === true);
      } else {
        doFishing();
      }
      break;
...
function connect(sessionToken) {
  ...
  state.ws = new WebSocket(GAME_WS, proxyAgent ? { agent: proxyAgent } : undefined);
...
async function main() {
  ...
  if (proxyConfig) {
    console.log(`  Proxy    : ${proxyConfig.url}`);
  } else {
